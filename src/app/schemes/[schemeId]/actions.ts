...
const extractLikelyUkPostcode = (value: string | null | undefined) => {
  ...
  if (direct.length >= 5 && direct.length <= 8) return direct;
  const match = source.match(UK_POSTCODE_FINDER_REGEX);
  if (!match?.[1]) return null;
  const extracted = normalizePostcode(match[1]);
  return extracted.length >= 5 && extracted.length <= 8 ? extracted : null;
};
...
export async function addSchemeProduct(
  ...
  const plantPostcode =
    extractLikelyUkPostcode(rawPlantPostcode) ?? normalizePostcode(rawPlantPostcode);
  const sitePostcode =
